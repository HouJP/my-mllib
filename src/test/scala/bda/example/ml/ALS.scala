  ...
  def train[ID: ClassTag]( // scalastyle:ignore
    ...
    if (implicitPrefs) {
      ...
      for (iter <- 0 until maxIter) {
        ...
        val t_iter = (t_end - t_start) / 1e6

        cost_time += t_iter
        cost_count += 1
        println(s"iter = $cost_count, time = $t_iter")
      }
      println(s"cost_count = $cost_count, cost_time = $cost_time, ave_cost_time = ${cost_time / cost_count}")
    }
